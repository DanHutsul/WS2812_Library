...
# 2 "../Core/Src/ws2812.c" 2

extern TIM_HandleTypeDef htim2;

uint16_t BUF_DMA [50 + 10*24] = {24};
...
void ws2812_pixel_rgb_to_buf_dma(uint8_t Rpixel , uint8_t Gpixel, uint8_t Bpixel, uint16_t posX)
{
  ...
  for(i=0;i<8;i++)
  {
    if (((Rpixel & (1<<(7-i))) != 0) == 1)
    {
      BUF_DMA[50 +posX*24+i+8] = 65;
    }else
    {
      BUF_DMA[50 +posX*24+i+8] = 24;
    }
    if (((Gpixel & (1<<(7-i))) != 0) == 1)
    {
      BUF_DMA[50 +posX*24+i+0] = 65;
    }else
    {
      BUF_DMA[50 +posX*24+i+0] = 24;
    }
    if (((Bpixel & (1<<(7-i))) != 0) == 1)
    {
      BUF_DMA[50 +posX*24+i+16] = 65;
    }else
    {
      BUF_DMA[50 +posX*24+i+16] = 24;
    }
  }
}
...
void ws2812_light(void)
{
  HAL_TIM_PWM_Start_DMA(&htim2,0x00000004U,(uint32_t*)&BUF_DMA,50 + 10*24);
}
